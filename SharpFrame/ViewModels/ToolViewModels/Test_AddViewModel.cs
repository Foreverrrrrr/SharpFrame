...
namespace SharpFrame.ViewModels.ToolViewModels
{
    public class Test_AddViewModel : BindableBase
    {
        ...
        public Test_AddViewModel(IEventAggregator aggregator, Structure.Parameter.TestParameter testParameter)
        {
            ...
            {
                Test_AddView currentWindow = System.Windows.Application.Current.Windows.OfType<Test_AddView>().SingleOrDefault(w => w.IsActive);
                currentWindow.Close();
            });
            OkButton = new DelegateCommand(() =>
            {
                Structure.Parameter.TestParameter structure = new Structure.Parameter.TestParameter();
                structure.Name = Name;
